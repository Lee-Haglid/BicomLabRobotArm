...
float halfToFloat(real16_T a)
{
  ...
  /* polyspace +3 MISRA2012:D4.1 CERT-C:INT31-C [Justified:Low] "(uint32_T)(~a.bitPattern)
...
(uint32_T)(~a.bitPattern) & 0x7C00U" */
  ...
  if (ExpComp == 0U) {
    ...
    /* Input is finite */
    ans = getFloatFromBitfield(OutSign | ((Exp != 0U) ? 1U : 0U) * ((Exp + 112U) << 23 | Mant) | (uint32_T)(((Exp == 0U) ? 1U : 0U) & ((Mant != 0U) ? 1U : 0U)) * ((LeadingZeros - 37U) << 23 | ((Mant << (150U - LeadingZeros)) & 0x007FE000U)));
  }
  return ans;
}
...
real16_T doubleToHalf(double a)
{
  ...
  if (aExponent == (uint16_T)(0x7FF00000UL >> (52-32))) {/* Inf or NaN input */
    outExponent = 0x7C00U;
    outMantissa = (aMantissaFirstChunk == 0 && aMantissaSecondChunk == 0) ? 0 : 0x0200U;
...
evaluates sticky bit. Other bit overflows do not have any effect here." */
      ...
      round_bit = ((aMantissaFirstChunk >> (41 - 32 + shift_length) & 0x00000001UL) != 0);
      outExponent = 0;
      outMantissa = (uint16_T)((aMantissaFirstChunk >> (42 - 32  + shift_length)));
    } else {
      sticky_bit = ((aMantissaFirstChunk & 0x000001FFUL) != 0 || aMantissaSecondChunk != 0);
