function ServoStreamerUIV7
% ServoStreamUI - 5-joint live queue streamer + HR send + IMU logging
%
...
%   MATLAB -> IMU_STOP
%
% Arduino -> IMU,time_ms,roll,pitch,yaw,gx,gy,gz,ax,ay,az
    ...
    fig = uifigure('Name','Arduino 5-Joint Live Streamer + IMU Logger', ...
        'Position',[100 100 820 610]);

    uilabel(fig,'Position',[20 555 80 22],'Text','CSV/XLSX:');
    fileField = uieditfield(fig,'text','Position',[100 555 520 22], ...
        'Value',state.filePath);

    uibutton(fig,'push','Position',[635 555 110 22],'Text','Browse...', ...
        'ButtonPushedFcn',@(~,~)safeRun(@onBrowseTrajectory));

    uilabel(fig,'Position',[20 515 50 22],'Text','Port:');
    portDrop = uidropdown(fig,'Position',[100 515 150 22], ...
        'Items',getPortsSafe(),'ValueChangedFcn',@onPortChange);

    uibutton(fig,'push','Position',[260 515 80 22],'Text','Refresh', ...
        'ButtonPushedFcn',@(~,~)safeRun(@onRefreshPorts));

    uilabel(fig,'Position',[360 515 45 22],'Text','Baud:');
    baudField = uieditfield(fig,'numeric','Position',[410 515 90 22], ...
        'Value',state.baud,'Limits',[1200 2000000]);

    baudField.RoundFractionalValues = true;
    baudField.ValueDisplayFormat = '%.0f';

    uilabel(fig,'Position',[20 475 110 22],'Text','Timestamp unit:');
    unitDrop = uidropdown(fig,'Position',[140 475 120 22], ...
        'Items',{'seconds','ms'},'Value',state.timeUnit);

    uilabel(fig,'Position',[300 475 110 22],'Text','Heart Rate BPM:');
    hrField = uieditfield(fig,'numeric','Position',[420 475 80 22], ...
        'Value',state.hrBpm,'Limits',[1 240]);

    hrField.RoundFractionalValues = true;
    hrField.ValueDisplayFormat = '%.0f';

    sendHrBtn = uibutton(fig,'push','Position',[520 475 100 22], ...
        'Text','Send HR','Enable','off', ...
        'ButtonPushedFcn',@(~,~)safeRun(@onSendHR));

    uilabel(fig,'Position',[20 435 90 22],'Text','IMU file:');
    imuFileField = uieditfield(fig,'text','Position',[100 435 520 22], ...
        'Value',state.imuPath);

    uibutton(fig,'push','Position',[635 435 110 22],'Text','Save As...', ...
        'ButtonPushedFcn',@(~,~)safeRun(@onBrowseIMUSave));

    connectBtn = uibutton(fig,'push','Position',[20 385 120 34], ...
        'Text','Connect','ButtonPushedFcn',@(~,~)safeRun(@onConnect));

    disconnectBtn = uibutton(fig,'push','Position',[150 385 120 34], ...
        'Text','Disconnect','Enable','off', ...
        'ButtonPushedFcn',@(~,~)safeRun(@onDisconnect));

    streamBtn = uibutton(fig,'push','Position',[280 385 140 34], ...
        'Text','Stream Live','Enable','off', ...
        'ButtonPushedFcn',@(~,~)safeRun(@onStreamLive));

    stopBtn = uibutton(fig,'push','Position',[430 385 90 34], ...
        'Text','Stop','Enable','off', ...
        'ButtonPushedFcn',@(~,~)safeRun(@onStop));

    zeroBtn = uibutton(fig,'push','Position',[530 385 80 34], ...
        'Text','Zero','Enable','off', ...
        'ButtonPushedFcn',@(~,~)safeRun(@onZero));

    imuStartBtn = uibutton(fig,'push','Position',[620 385 140 34], ...
        'Text','Get Motion Data','Enable','off', ...
        'ButtonPushedFcn',@(~,~)safeRun(@onStartIMU));

    imuStopBtn = uibutton(fig,'push','Position',[620 345 140 34], ...
        'Text','Stop Motion Data','Enable','off', ...
        'ButtonPushedFcn',@(~,~)safeRun(@onStopIMU));

    uilabel(fig,'Position',[20 345 100 22],'Text','Arduino log:');
    logBox = uitextarea(fig,'Position',[20 20 760 320],'Editable','off');
    ...
    function onBrowseTrajectory
        [f,p] = uigetfile( ...
            {'*.xlsx;*.xls;*.csv','Trajectory Files (*.xlsx,*.xls,*.csv)';'*.*','All Files'}, ...
    ...
    function onBrowseIMUSave
        ...
        logMsg("Selected IMU save file: " + state.imuPath);
    end
    ...
    function onStartIMU
        ...
        if state.imuRecording
            logMsg("IMU recording already running.");
            return;
        end

        state.imuPath = string(imuFileField.Value);

        if strlength(state.imuPath) == 0
            error("No IMU save file selected.");
        end

        outDir = fileparts(state.imuPath);

        if strlength(outDir) > 0 && ~isfolder(outDir)
            error("IMU save folder does not exist.");
        end

        state.imuFID = fopen(state.imuPath, 'w');

        if state.imuFID < 0
            error("Could not open IMU file for writing.");
        end
    ...
    function onStreamLive
        ...
        try
            ...
            sentTbl = table( ...
                ...
                'VariableNames', { ...
                    't_ms_full','bottom_raw','mid_raw','top_raw','elbow_raw','wrist_raw', ...
    ...
    function writeIMULine(s)
        ...
        try
            ...
            logMsg("IMU WRITE ERROR: " + string(ME.message));
        end
    end
